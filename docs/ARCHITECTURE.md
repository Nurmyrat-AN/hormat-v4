...
- System/domain folders, including users and cache, can be renamed/deleted by authorized users, with reference-risk warnings. No permission implies unused/reference safety. Cache owner `record.json`, hidden work areas and writes inside `cache/<UUID>/` token directories remain reserved infrastructure; ordinary cache folders/files remain manageable. Manual files, even token-looking filenames, gain no domain metadata/ownership or finalization rights.
...
- Media navigation may be enabled with `media.view` only after mutation/security, Universal Media, full regression and production verification pass. Availability is not a permission.
