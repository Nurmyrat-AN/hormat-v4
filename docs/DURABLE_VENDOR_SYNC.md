...
Products referenced by earlier historical transactions can be fetched with authenticated Nano `_all_docs` bulk POSTs by exact keys. Product currency/measure dependencies are resolved in a second bounded round. When the reserved `z_walyuta-1` currency is missing, that same round also requests `ayar_umum-1`, its approved settings source. This applies after checkpoint resume as well as initial bootstrap; a late settings document can unblock the existing batch without a reset. Missing settings still fail atomically, and dependency timestamps never advance stream activity. Known IDs are queried in bulk from PostgreSQL, strictly within the current Vendor. There are no placeholder entities and no permanent raw-document queue.
