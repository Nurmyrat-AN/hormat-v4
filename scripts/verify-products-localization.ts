...
try{
 actor=(await bootstrapSuperuser({name:'Products localization verification',email:randomUUID()+'@example.invalid',password:randomUUID()})).id;
 ...
 for(const language of ['tm','ru','en']){
  let corpus='';
  for(const route of ['/cpanel/products']){
  ...
  console.info('PASS Products live '+language+': page, actual DB values, topbar language return routes');
 }
