...
try{
 actor=(await bootstrapSuperuser({name:'Currency localization verification',email:randomUUID()+'@example.invalid',password:randomUUID()})).id;
 ...
 const keys=[...new Set((await Promise.all(['src/views/cpanel/pages/currencies.ejs','src/views/cpanel/pages/currencies-content.ejs','src/views/cpanel/partials/content/visibility.ejs','src/views/cpanel/partials/content/translatable-field.ejs'].map(file=>readFile(file,'utf8')))).flatMap(keysInSource))];
 ...
 for(const language of ['tm','ru','en']){
  let corpus='';
  for(const route of ['/cpanel/currencies/frontend','/cpanel/currencies/vendors']){
  ...
  console.info('PASS Currencies live '+language+': both pages, actual DB values, topbar language return routes');
 }
