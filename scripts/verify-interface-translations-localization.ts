...
try{
 actor=(await bootstrapSuperuser({name:'Interface translations localization verification',email:randomUUID()+'@example.invalid',password:randomUUID()})).id;
 ...
 const keys=[...new Set((await Promise.all(['src/views/cpanel/pages/interface-translations.ejs','src/views/cpanel/pages/interface-translations-content.ejs'].map(file=>readFile(file,'utf8')))).flatMap(keysInSource))];
 ...
 for(const language of ['tm','ru','en']){
  let corpus='';
  for(const route of ['/cpanel/interface-translations']){
   ...
   await page.locator('#translations-search').fill('__verification_no_results__');await page.waitForTimeout(400);
  ...
  console.info('PASS Interface Translations live '+language+': page, actual DB values, topbar language return routes');
 }
