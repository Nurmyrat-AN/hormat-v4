...
try{
 actor=(await bootstrapSuperuser({name:'Languages localization verification',email:randomUUID()+'@example.invalid',password:randomUUID()})).id;
 ...
 const keys=[...new Set((await Promise.all(['src/views/cpanel/pages/languages.ejs','src/views/cpanel/pages/languages-content.ejs'].map(file=>readFile(file,'utf8')))).flatMap(keysInSource))];
 ...
 for(const language of ['tm','ru','en']){
  let corpus='';
  for(const route of ['/cpanel/languages']){
  ...
  console.info('PASS Languages live '+language+': page, actual DB values, topbar language return routes');
 }
