...
try{
 actor=(await bootstrapSuperuser({name:'Source Products localization verification',email:randomUUID()+'@example.invalid',password:randomUUID()})).id;
 ...
 const keys=[...new Set((await Promise.all(['src/views/cpanel/pages/source-products.ejs','src/views/cpanel/pages/source-products-content.ejs','src/views/cpanel/partials/content/async-autocomplete.ejs'].map(file=>readFile(file,'utf8')))).flatMap(keysInSource))];
 ...
 for(const language of ['tm','ru','en']){
  let corpus='';
  for(const route of ['/cpanel/source-products']){
   ...
   await page.locator('#source-filters-toggle').click();await page.waitForFunction(()=>document.querySelectorAll('[data-source-details]').length>0);await page.locator('[data-source-details]').first().click();
  ...
  console.info('PASS Source Products live '+language+': page, actual DB values, topbar language return routes');
 }
