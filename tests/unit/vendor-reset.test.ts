...
async function fixture(){const v=await vendor();await apply(v,[...references(),product(),movement('purchase',101,10),movement('sale',201,3)]);await db.query('INSERT INTO products(source_product_id) SELECT id FROM source_products WHERE vendor_id=$1',[v.id]);return v;}
...
test('production HTTP reset: real CSRF/auth, stopped feed, committed empty state, Nano replay and stable FK', {timeout:20000},async()=>{
 ...
 const v=await vendor();v.url=`http://127.0.0.1:${(couch.address() as any).port}/fixture`;await db.query('UPDATE vendors SET url=$2 WHERE id=$1',[v.id,v.url]);await apply(v,documents);await db.query('INSERT INTO products(source_product_id) SELECT id FROM source_products WHERE vendor_id=$1',[v.id]);const before=await snapshot(v);
