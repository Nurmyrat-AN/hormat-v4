 ...
 try{
  ...
  for(const [permission,operation] of [['media.upload','files'],['media.create_folder','folders'],['media.rename','rename'],['media.delete','delete']]){
   ...
   if(operation==='files'){
    ...
    for(const name of ['../bad','bad/name','bad\\name','/tmp/bad','C:bad','.hidden','%2e%2e'])expect((await upload(name)).status()).toBe(400);
...
test('Media real UI: per-file success/error, retry, search refresh, recursive confirmation and translated outcomes',async({page,context,baseURL})=>{
 ...
 try{
  ...
  for(const language of ['tm','ru','en']){
   ...
   await page.locator('[data-media-action=new]').click();await page.locator('#media-name').fill('match-folder');await page.locator('#media-dialog-confirm').click();await expect(page.locator('#media-feedback')).toHaveText(t('created'));await expect(page.locator('.media-item')).toHaveCount(3);
   await writeFile(path.join(root,'match-folder/child'),'child');const target=page.locator('.media-item').filter({hasText:'match-folder'});await target.locator('[data-bs-toggle=dropdown]').click();await target.locator('[data-media-action=delete]').click();await expect(page.locator('#media-recursive')).toBeVisible();await expect(page.locator('#media-dialog-confirm')).toBeDisabled();await expect(page.locator('#media-recursive')).toContainText(t('nonEmpty'));
   for(const theme of ['light','dark']){await page.evaluate(theme=>document.documentElement.dataset.bsTheme=theme,theme);for(const width of [1440,375]){await page.setViewportSize({width,height:900});expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBe(width);await page.screenshot({path:`artifacts/media-activation-${language}-${theme}-${width}.png`,fullPage:true});}}
