...
for(const language of ['tm','ru','en'])test(`Media manager ${language}: real browsing, mock actions, selection, themes and mobile`,async({page,context,baseURL})=>{
 ...
 try{
  ...
  await page.locator('[data-media-action=new]').click();await page.locator('#media-name').fill('Do not create');await page.locator('#media-dialog-confirm').click();await expect(page.locator('#media-feedback')).toBeVisible();
  await page.locator('[data-media-action=upload]').click();await page.locator('#media-dialog-confirm').click();await expect(page.locator('#media-queue-error')).toBeVisible();await page.locator('#media-files').setInputFiles({name:'preview.txt',mimeType:'text/plain',buffer:Buffer.from('preview')});await expect(page.locator('#media-queue')).toContainText('preview.txt');await page.locator('#media-dialog-confirm').click();await expect(page.locator('#media-queue progress')).toHaveJSProperty('value',100);await page.locator('#media-dialog .btn-close').click();
  for(const action of ['rename','delete']){const item=page.locator('.media-item').filter({hasText:'Привет'});await item.locator('[data-bs-toggle=dropdown]').click();await item.locator(`[data-media-action=${action}]`).click();if(action==='rename')await page.locator('#media-name').fill('changed.txt');await page.locator('#media-dialog-confirm').click();}
  expect(posts).toBe(0);expect((await readdir(root)).sort()).toEqual(['Nested','Привет Türkmen.txt',imageName].sort());expect(await readFile(path.join(root,'Привет Türkmen.txt'),'utf8')).toBe('unchanged');
...
test('Media read authorization, strict grants, no manager mutation endpoints and bounded path validation',async({page,context,baseURL,browser})=>{
 ...
 try{
  ...
  for(const action of ['rename','delete','folder'])expect((await context.request.post('/cpanel/api/media/'+action,{headers:{Cookie:cookie,'X-CSRF-Token':session.session.csrf_token},data:{}})).status()).toBe(404);
...
test('Media continuation: history, refresh, selected search results, details and missing-folder recovery',async({page,context,baseURL})=>{
 ...
 try{
  ...
  await page.locator('.media-item.is-selected .media-name').click();await expect(page.locator('[data-detail=mime]')).toHaveText('image/png');await expect(page.locator('[data-detail=dimensions]')).toHaveText('1 × 1');await expect(page.locator('#media-detail-preview')).toBeVisible();await page.locator('#media-dialog [data-media-action=rename]').click();await expect(page.locator('#media-name')).toHaveValue(name);await page.locator('#media-dialog .btn-close').click();
