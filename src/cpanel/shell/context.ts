...
export const shellContext: RequestHandler = async (request, response, next) => {
  ...
  const canViewProducts=await response.locals.permissions.hasPermission('products.view');
