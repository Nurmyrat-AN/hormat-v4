(() => {
 ...
 let modalReady=false;dialog.addEventListener('show.bs.modal',()=>{modalReady=false;});dialog.addEventListener('shown.bs.modal',()=>{modalReady=true;});
 function closeDialog(){if(modalReady)modal.hide();else dialog.addEventListener('shown.bs.modal',()=>modal.hide(),{once:true});}
 ...
 document.addEventListener('click',event=>{
  ...
  document.querySelector('#media-dialog-item').textContent=['rename','move','delete','details'].includes(action)?selected?.name??'':'';
  ...
  document.querySelector('#media-risk').hidden=!['rename','delete'].includes(action);
  ...
  if(action==='move'){confirm.disabled=true;loadMoveFolders(selected.parent);}
 ...
 async function loadDeleteInfo(location){
  detailRequest=new AbortController();const active=detailRequest;
  try{const response=await fetch('/cpanel/api/media/delete-info?'+new URLSearchParams({path:location}),{signal:active.signal});const data=await response.json();if(!response.ok||!data.success)throw Object.assign(new Error(errorText(data.error?.code,response.status)),{localized:true});
  ...
  }catch(error){if(error.name!=='AbortError'&&active===detailRequest)message(error.localized?error.message:root.dataset.failed);}
 }
 ...
 let moveDestination='',moveReady=false;
 function invalidDestination(location){return location===selected.parent?'MEDIA_SAME_FOLDER':selected.folder&&(location===selected.path||location.startsWith(selected.path+'/'))?'MEDIA_MOVE_SELF':null;}
 async function loadMoveFolders(location){
  detailRequest?.abort();detailRequest=new AbortController();const active=detailRequest;
  moveReady=false;confirm.disabled=true;confirm.textContent=mutationLabels.moveHere;
  const folders=document.querySelector('#media-move-folders'),crumbs=document.querySelector('#media-move-breadcrumb'),notice=document.querySelector('#media-move-notice');
  folders.replaceChildren();crumbs.replaceChildren();notice.textContent=mutationLabels.loading;
  document.querySelector('#media-move-destination').textContent=location||mutationLabels.root;
  try{
   const response=await fetch('/cpanel/api/media/move-folders?'+new URLSearchParams({path:location}),{signal:active.signal});
   const data=await response.json();if(!response.ok||!data.success)throw Object.assign(new Error(errorText(data.error?.code,response.status)),{localized:true});
   if(active!==detailRequest||action!=='move')return;
   moveDestination=data.path;moveReady=true;
   const navigate=(name,path,container)=>{const button=document.createElement('button');button.type='button';button.className=container===folders?'list-group-item list-group-item-action text-break':'btn btn-sm btn-outline-secondary text-break';button.textContent=name;button.dataset.destination=path;button.addEventListener('click',()=>{if(!busy)loadMoveFolders(path);});container.append(button);return button;};
   navigate(mutationLabels.root,'',crumbs);let prefix='';for(const name of data.path.split('/').filter(Boolean)){prefix=[prefix,name].filter(Boolean).join('/');navigate(name,prefix,crumbs);}
   for(const folder of data.folders){const button=navigate(folder.name,folder.path,folders);if(selected.folder&&(folder.path===selected.path||folder.path.startsWith(selected.path+'/'))){button.disabled=true;button.title=errors.MEDIA_MOVE_SELF;}}
   const invalid=invalidDestination(data.path);notice.textContent=invalid?errors[invalid]:data.limited?mutationLabels.limited:'';confirm.disabled=!!invalid;
   document.querySelector('#media-cache-warning').hidden=!(selected.temporary||data.path.split('/')[0]==='cache');
  }catch(error){if(error.name!=='AbortError'&&active===detailRequest){notice.textContent='';message(error.localized?error.message:errors.MEDIA_MOVE_FAILED);}}
 }
 function setBusy(value){busy=value;confirm.disabled=value;confirm.textContent=value?mutationLabels.working:action==='move'?mutationLabels.moveHere:mutationLabels.confirm;dialog.querySelectorAll('[data-bs-dismiss],#media-files,#media-name').forEach(el=>el.disabled=value);}
 ...
 document.querySelector('#media-dialog-form').addEventListener('submit',async event=>{
  ...
  if(action==='move'&&(!moveReady||invalidDestination(moveDestination)))return;
  if(action==='upload'){
  ...
  const payload=action==='new'?{parent:form.elements.path.value,name:document.querySelector('#media-name').value}:action==='rename'?{path:selected.path,name:document.querySelector('#media-name').value}:action==='move'?{path:selected.path,destination:moveDestination}:{path:selected.path,recursive:document.querySelector('#media-recursive-confirm').checked};
  setBusy(true);
  try{
   ...
   const data=await response.json().catch(()=>({}));if(!response.ok||!data.success)throw Object.assign(new Error(errorText(data.error?.code,response.status)),{code:data.error?.code,localized:true});
   const text=mutationLabels[action==='new'?'created':action==='rename'?'renamed':action==='move'?'moved':'deleted'];await search();setBusy(false);closeDialog();message(text);
  }catch(error){setBusy(false);message(error.localized?error.message:root.dataset.failed);if(error.code==='MEDIA_FOLDER_NOT_EMPTY')loadDeleteInfo(selected.path);if(action==='move'&&error.code==='MEDIA_MOVE_INCOMPLETE'){await search();moveReady=false;confirm.disabled=true;message(error.message);}}
 });
